    ...
    while opt_iter < opt["max_iter"] and change > opt["opt_tol"]:
        ...
        if fem_params["hyperelastic"]:
            ...
            for step in range(1, N_steps + 1):
                ...
                # Solve with current u_field as initial guess
                femProblem.solve_fem()
        
                # Optional: print per-step displacement
                u_array = u_field.x.array
                max_disp = np.max(np.abs(u_array))
               # if comm.rank == 0:
                  #  print(f"Step {step}/{N_steps}, max abs displacement: {max_disp:.4e}")
        
        else: # Solve linear FEM problem       
            femProblem.solve_fem() 
    
        
        u_norm = u_field.x.petsc_vec.norm()
        u_array = u_field.x.array
        max_disp = np.max(np.abs(u_array))
        #if comm.rank == 0:
         #   print(f"max abs displacement: {max_disp:.4e}")
        ...
        #if opt["opt_compliance"]:
            # Use strain energy as objective instead of compliance
         #   dJdrho = dUdrho
          #  g_vec = np.array([V_value - opt["vol_frac"]])
           # dgdrho = np.vstack([dVdrho])
        else:
