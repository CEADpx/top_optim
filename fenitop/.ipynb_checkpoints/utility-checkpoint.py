...
class WrapNonlinearProblem:
    def __init__(self, u, R, bcs=[], petsc_options={}):
        """
        This class wraps together the nonlinear residual problem and the solver to work 
        with .solve_fem(), hopefully
        """
        self.u = u  #unknown displacement field to solve for
        ...
        for k, v in petsc_options.items():
            ...
            PETSc.Options().setValue(k, v) #loop stores user-defined PETSc options like 'cg' or 'gamg' globally
        self.solver.krylov_solver.setFromOptions()  #apply options to PETSc Krylov solver (inner linear solver each iteration)

    def solve_fem(self):
        """
        Runs the NewtonRaphson solve to find a u that makes R(u) ≈ 0
        if it doesn’t converge, throw an error
        """
        #print("Calling Newton solver...", flush=True)     #ADDED
        #num_its, converged = self.solver.solve(self.u)
        #assert converged, f"Newton solver failed to converge in {num_its} iterations"
        ...
        #print(f"[Newton Solve] Converged: {converged}")
        #print(f"[Newton Solve] Iterations: {num_its}")
    
        # If you want to forcibly stop if not converged
...
def plot_design(mesh, density, tag = None, path="", threshold=0.49, smooth_iter=100, pv_or_image = "pv"):
  ...
  #  if dim == 2:
   ...
   # if pv_or_image == "image":
     #   image = pyvista.read(filename + ".png")
    #    image.plot(rgb=True, cpos='xy')
   # elif pv_or_image == "pv":
        #plotter.show(interactive = False, jupyter_backend='static')
